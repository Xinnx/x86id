...
getbrandstring(struct cpuinfo_s* cpuinfo) {
  ...
  //
  char* curr = &str[0];
  uint org_len = strlen(str);
  uint space_count = 0;
  while(!isalpha(*curr)) {
    curr++;
    space_count++;
  }
  uint total_len = (org_len - space_count);
...
getbasicinfo(struct cpuinfo_s* cpuinfo) {
  ...
  if (cpuinfo->u_model_info.b.BaseFamily == 0xF) {
    ...
    cpuinfo->family = cpuinfo->u_model_info.b.BaseFamily;
  } else {
    cpuinfo->family = (cpuinfo->u_model_info.b.BaseFamily +
  ...
  if (!(cpuinfo->u_model_info.b.BaseFamily == 0xF
    ...
    cpuinfo->model = cpuinfo->u_model_info.b.BaseModel;
  } else {
      cpuinfo->model = 
        ((cpuinfo->u_model_info.b.ExtModel) << 4) + 
...
main() {
  ...
  if(checkcpuid()) {
    ...
    struct cpuinfo_s* cpuinfo = malloc(sizeof(struct cpuinfo_s));
    cpuinfo->flevel = 0x0;
    cpuinfo->cpuvend[0] = '\0';
